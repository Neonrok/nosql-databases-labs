...
const users = exercisesDb.users;

users.drop();

users.insert({ _id: "jane", joined: ISODate("2011-03-02"), likes: ["golf", "racquetball"] });
users.insert({ _id: "joe", joined: ISODate("2012-07-02"), likes: ["tennis", "golf", "swimming"] });
users.insert({ _id: "jill", joined: ISODate("2012-09-02"), likes: ["golf", "racquetball"] });
users.insert({
  _id: "ruth",
  joined: ISODate("2011-01-12"),
  likes: ["swimmig", "golf", "racquetball"],
});
users.insert({ _id: "kate", joined: ISODate("2012-01-23"), likes: ["tennis", "darts"] });
users.insert({
  _id: "harold",
  joined: ISODate("2012-01-15"),
  likes: ["golf", "basketball", "darts", "tennis"],
});

// 1 normalize usernames: in upper case and in alphabetical order
users.aggregate([{ $project: { name: { $toUpper: "$_id" }, _id: false } }, { $sort: { name: 1 } }]);
