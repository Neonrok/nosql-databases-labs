...
const laptops = exercisesDb.laptops;
...
laptops.insert({
  ...
  properties: {
    ...
    HD: "850GB",
    OS: "Mac OS X 10.11",
    USB: { USB2_0: 0, USB3_0: 2, USB3_1: 0 },
  },
  LanguageOS: ["Dutch", "French"],
  software: ["iMovie", "GarageBand", "iBooks", "Safari", "FaceTime"],
  reviews: [{ by: "Tom", score: 4.4, text: "Superb" }],
});

// 1 Give all laptops with a price between 500 and 1000 euro or with a hard disk of 750 GB
laptops.find({
  $or: [{ price: { $gte: 500, $lte: 1000 } }, { "properties.HD": "750GB" }],
});

// 2 Change Windows 10 to Windows 8
laptops.updateMany({ "properties.OS": "Windows 10" }, { $set: { "properties.OS": "Windows 8" } });

// 3 Give name, price and operating system of the 2 cheapest laptops
laptops.find({}, { name: true, price: true, "properties.OS": true }).sort({ price: 1 }).limit(2);

// 4 Give all laptops with at least 1 review with a score between 4 and 4.2
laptops.find({ "reviews.score": { $gte: 4, $lte: 4.2 } });
